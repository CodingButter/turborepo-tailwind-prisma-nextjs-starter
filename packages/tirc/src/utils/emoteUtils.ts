...
const convertTwitchEmote = (emote: ITwitchEmote): IEmote =>
	({
		...
		urls: {
			...
			'3x':
				emote.images.url_3x || emote.images.url_2x || emote.images.url_1x || '',
			'4x':
				emote.images.url_4x ||
				emote.images.url_3x ||
				emote.images.url_2x ||
...
export const fetchFFZGlobalEmotes = async (): Promise<IEmote[]> => {
	try {
		...
		}>(`${FFZ_API_BASE}/set/global`, {}, 'Error fetching FFZ global emotes');
 ...
 */
export const fetchFFZChannelEmotes = async (
	channelId: string,
): Promise<IEmote[]> => {
	try {
		const data = await safeApiFetch<{
 ...
 */
export const formatMessageWithEmotes = (
	message: string,
	emotes: IEmote[],
): string => {
 ...
 * @param emotes - The list of available emotes.
 */
export const extractEmotesFromMessage = (
	message: string,
	emotes: IEmote[],
): IEmote[] => {
...
export const parseEmotes = (
	...
	return emoteString
		...
		.filter(
			(
				emote,
			): emote is { id: string; code: string; positions: EmotePosition[] } =>
				emote !== null,
		);
};
 ...
 */
export const getTwitchEmoteUrl = (
	id: string,
	size: '1.0' | '2.0' | '3.0' = '1.0',
): string => {
...
export const splitMessageWithEmotes = (
	...
	const mergedRanges = ranges.reduce<
		Array<{ start: number; end: number; emote: EmoteInfo }>
	>((acc, range) => {
		if (acc.length === 0) {
			acc.push(range);
			return acc;
