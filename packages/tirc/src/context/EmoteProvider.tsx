...
export const EmoteProvider: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	...
	const fetchEmotes = useCallback(
		async (channelId?: string) => {
			...
			if (fetchCountRef.current > 10) {
				console.warn(
					'Too many emote fetch attempts, aborting to prevent infinite loop',
				);
				return;
			}
			...
			try {
				...
				if (channelId) {
					try {
						const bttvChannel = await fetchBTTVChannelEmotes(
							'twitch',
							channelId,
						);
						console.log(
							`Fetched ${bttvChannel.length} BTTV channel emotes for channel ${channelId}`,
						);
						allEmotes.push(...bttvChannel);

						const ffzChannel = await fetchFFZChannelEmotes(channelId);
						console.log(
							`Fetched ${ffzChannel.length} FFZ channel emotes for channel ${channelId}`,
						);
	...
	const getEmote = useCallback(
		(name: string): IEmote | undefined => {
			return emotes.find(
				(emote) => emote.name.toLowerCase() === name.toLowerCase(),
			);
		},
		[emotes],
	);
