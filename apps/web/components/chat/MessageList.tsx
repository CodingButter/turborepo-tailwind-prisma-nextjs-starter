...
export const MessageList: React.FC<MessageListProps> = ({
	...
	useEffect(() => {
		...
		if (filters.onlyFromUser) {
			result = result.filter(
				(msg) =>
					msg.username.toLowerCase() === filters.onlyFromUser?.toLowerCase(),
			);
		}
	...
	const handleUsernameClick = (username: string) => {
		if (onUsernameClick) {
			...
			if (filters.onlyFromUser === username) {
				...
				setFilters((prev: ChatFilters) => ({
					...prev,
					onlyFromUser: username,
				}));
			}
		}
	};
		...
		return (
			<div className="flex items-center justify-center h-full">
				<div className="text-center p-8 bg-surface/50 rounded-lg border border-border max-w-md">
					...
					<p className="text-text text-xl font-medium mb-2">
						No channel selected
					</p>
