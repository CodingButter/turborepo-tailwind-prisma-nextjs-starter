...
const EmotesPicker: React.FC<EmotesPickerProps> = ({
	onClose,
	onSelectEmote,
	channelName,
}) => {
	...
	const [activeTab, setActiveTab] = useState<
		'twitch' | 'bttv' | 'ffz' | 'recent'
	>('twitch');
	const pickerRef = useRef<HTMLDivElement>(null);

	// Use useStorage for recent emotes instead of useState + localStorage
	const [recentEmotes, setRecentEmotes] = useStorage<string[]>(
		[],
		'recent-emotes',
	);
	...
	const twitchEmotes = emotes.filter(
		(emote) =>
			emote.name.startsWith('twitch_') ||
			(emote.id && emote.id.includes('twitch')),
	);

	const bttvEmotes = emotes.filter(
		(emote) =>
			emote.name.startsWith('bttv_') || (emote.id && emote.id.includes('bttv')),
	);

	const ffzEmotes = emotes.filter(
		(emote) =>
			emote.name.startsWith('ffz_') || (emote.id && emote.id.includes('ffz')),
	);
			...
			<div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-8 gap-2 p-2">
				...
				{emoteList.length > maxEmotesToRender && (
					<div className="col-span-full text-center py-2 text-text-secondary text-sm">
						Showing {maxEmotesToRender} of {emoteList.length} emotes. Refine
		...
		>
			...
			<div className="p-2 border-b border-border">
				<div className="relative">
					...
					<Search
						size={16}
						className="absolute left-2.5 top-2 text-text-secondary"
					/>
				</div>
			</div>
			...
			<div className="max-h-64 overflow-y-auto">
				{activeTab === 'twitch' &&
					renderEmoteGrid(filteredTwitchEmotes, 'twitch')}
			...
			<div className="p-2 bg-background-secondary border-t border-border text-xs text-text-tertiary flex justify-between items-center">
				...
				<div>
					{isLoading
						? 'Loading emotes...'
						: `${emotes.length} emotes available`}
				</div>
			</div>
		</div>
	);
