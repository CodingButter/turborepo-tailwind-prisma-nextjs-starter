		...
		>
			...
			<div className="flex-1 overflow-y-auto p-2 message-container">
				{channels.length > 0 ? (
							...
							(channel: Channel) =>
								channel && (
									...
									>
										...
										>
											{collapsed
												? channel.substring(1, 2).toUpperCase()
												: channel}
										</button>
										{!collapsed && (
											<div className="pr-2">
					...
					<p
						className={`text-text-secondary text-sm ${collapsed ? 'text-center' : ''}`}
					>
			...
			{(!collapsed || showJoinForm) && (
				<div
					className={`${collapsed ? 'absolute left-16 bottom-0 w-64 z-10' : ''}`}
				>
					<JoinForm
						...
						leaveCurrentChannel={() =>
							currentChannel && leaveChannel(currentChannel)
						}
